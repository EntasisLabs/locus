#!/usr/bin/env bash
# Build (and optionally push) the locus-mcp Docker image.
#
# Usage:
...
BUILD_FEATURES="${LOCUS_MCP_BUILD_FEATURES:-}"
NAME_SUFFIX="${LOCUS_MCP_BUILD_NAME_SUFFIX:-}"
BASE_IMAGE_NAME="locus-mcp"
...
mkdir -p "$PUBLISH_DIR"
BIN_PATH="$(resolve_binary_path "release/locus-mcp")"
cp "$BIN_PATH" "$PUBLISH_DIR/locus-mcp"
chmod +x "$PUBLISH_DIR/locus-mcp"

if command -v strip >/dev/null 2>&1; then
  strip "$PUBLISH_DIR/locus-mcp" || true
fi
