...
classDiagram
    ...
    class MemoryCompositionService {
      +recall_with_explain(request)
      +daily_rollup(request)
      +transform_then_recall_verify(providers, request)
    ...
    MemoryCompositionService --> MemoryRecallService
    MemoryCompositionService --> MemoryExplainService
    MemoryCompositionService --> MemoryAggregateService
```
