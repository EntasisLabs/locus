#!/usr/bin/env bash
# Build locus-cli release artifacts for multiple targets.
#
# Usage:
#   ./build.sh [--publish] [--features <csv>] [--name-suffix <suffix>]
#
# Environment:
#   LOCUS_CLI_VERSION   Artifact version (default: Cargo.toml version)
#   LOCUS_CLI_BUILD_FEATURES Optional cargo feature set
#   LOCUS_CLI_BUILD_NAME_SUFFIX Optional artifact/release suffix (for example: embeddings)
#
...
# Override order: LOCUS_CLI_VERSION -> Cargo.toml version -> 0.1.0
CARGO_VERSION="$(sed -n 's/^version = "\([^"]*\)"$/\1/p' "$MANIFEST_PATH" | head -n1 || true)"
VERSION="${LOCUS_CLI_VERSION:-${CARGO_VERSION:-0.1.0}}"

BASE_NAME="locus-cli"
BINARY_NAME="locus-cli"
BUILD_FEATURES="${LOCUS_CLI_BUILD_FEATURES:-}"
NAME_SUFFIX="${LOCUS_CLI_BUILD_NAME_SUFFIX:-}"
