...
if [[ "${LOCUS_SKIP_TESTS:-0}" != "1" ]]; then
  echo "[release] Preflight: cargo test (core, sdk, gateway, mcp, cli)"
  run cargo test -p locus-core-rs -p locus-sdk -p locus-gateway -p locus-mcp -p locus-cli
else
...
if $DO_BUILD; then
  ...
  echo "[release] SDK crates.io publish (manual step after core is live):"
  echo "  ./locus-sdk/publish-crates.sh"
...
[release] Post-build checklist
  ...
  3. Publish crates (core first, then sdk):
